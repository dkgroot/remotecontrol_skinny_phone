...
class SccpLogger:
    ...
    def connect(self, timeout=None, maxread=200):
      try:
        ...
        self.ssh = spawn('./ssh -y -y -s %s@%s' %(self.username, self.hostname), timeout=timeout, maxread=maxread)
...
if __name__ == '__main__':
    ...
    events = {
        re.compile(b'Read Sccp Length: (?P<length>\d*) messageType: (?P<type>0x\d*) avalable:'):handle_read,
        re.compile(b'Writing Sccp Length: (?P<length>\d*) for SCCP MSG: (?P<type>0x\d*)\\n'):handle_write,
